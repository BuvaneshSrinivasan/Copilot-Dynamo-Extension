...
**Malformed classifier JSON:** `SpecGeneratorService.Parse()` (`DynamoCopilot.Core/Services/SpecGeneratorService.cs`) occasionally receives a `TYPE: SPEC|{...}` response where the LLM drops the opening quote on an object key right after a comma (e.g. `,options":[...]` instead of `,"options":[...]`) â€” most often on the 2nd/3rd item in the `questions` array. `Parse()` retries with `MissingKeyQuoteRegex` (`([{,])(\w+)":` â†’ inserts the missing quote) before giving up. If the JSON is still unparseable after that, it returns `ChatText = null` (not the raw string) so `SendMessageCoreAsync` falls through to normal streaming code generation instead of dumping the broken `TYPE: SPEC|{...}` wire text into the chat as if it were a real response. **Do not** revert this fallback to `ChatText = raw` â€” that reintroduces the bug of leaking classifier wire-format text to the user.
...
### System prompt & Revit version awareness (Copilot only)

`SystemPromptFactory.Build(pythonEngine, ragContext, revitYear)` (`DynamoCopilot.Core/Services/SystemPromptFactory.cs`) builds the system prompt. `revitYear` is detected once per `CopilotPanelViewModel` construction via `RevitEnvironmentInterop.TryGetRevitYear()` (`DynamoCopilot.GraphInterop/RevitEnvironmentInterop.cs`) and cached in `_revitYear` — it is **not** re-detected per message. When known, it's injected as a `## Revit Version` section telling the model to only use members valid for that version (e.g. the Revit 2024 `ElementId.IntegerValue` → `ElementId.Value` long-ElementId change); when unknown, the model is told to flag version-sensitive members explicitly instead of assuming the newest API.

`RevitEnvironmentInterop.TryGetRevitYear()` is shared between both extensions — `SuggestNodesViewExtension` uses it to scope `ObsoleteNodeStore` (see below), and `DynamoCopilotViewExtension`/`CopilotPanelViewModel` use it for the system prompt. **Do not** reintroduce a private per-extension copy of this reflection logic — it lives in `GraphInterop` specifically so both extensions share one implementation.

The prompt also carries an `## Accuracy Requirements` section: only use officially documented API members, never invent methods/properties/overloads to fill a gap, and state uncertainty explicitly in the response text rather than guessing silently. This is a prompt-level instruction only — `RevitApiSurfaceValidator` below is the deterministic backstop for when the model doesn't comply.

### Code validation & auto-fix (Copilot only)

After each streamed response, if `_settings.EnableCodeValidation` and a code block was extracted, `CopilotPanelViewModel.ValidateGeneratedCode()` runs two static validators against the code and merges their issues:

- **`RevitEnumValidator`** (`DynamoCopilot.Core/Services/Validation/RevitEnumValidator.cs`) â€” reflects on the currently loaded `RevitAPI.dll` to check that every `BuiltInParameter.X` / `BuiltInCategory.X` / `UnitTypeId.X` referenced in the generated code actually exists in this Revit installation.
- **`RevitApiSurfaceValidator`** (`DynamoCopilot.Core/Services/Validation/RevitApiSurfaceValidator.cs`) â€” catches invented methods/properties on Revit's static helper classes (e.g. `UnitUtils`, `LabelUtils`, `WallUtils`). It reflects over the loaded `RevitAPI`/`RevitAPIUI`/`RevitAPIIFC` assemblies, indexes every C# **static class** (`IsAbstract && IsSealed`, not an enum) under the `Autodesk.Revit` namespace, and flags any `ClassName.Member` reference where `ClassName` is a known static helper but `Member` isn't a real public static method/property/field on it. Scope is deliberately limited to static-class member access â€” instance calls (e.g. `wall.Location`) can't be checked without real Python type inference, so they're left alone to avoid false positives on local variable names.

Both validators return `ValidationResult`/`ValidationIssue` (`Category` is `"BuiltInParameter"`, `"BuiltInCategory"`, `"UnitTypeId"`, or `"ApiMember"`). If issues are found, `RunAutoFixLoopAsync` re-prompts the LLM via `AutoFixRequestBuilder.Build(code, result, attempt)` (2 attempts, escalating from "fix just these values" to "rewrite around them entirely"); if still invalid after both attempts, a warning banner is shown instead of silently failing.

**Do not** extend `RevitApiSurfaceValidator` to check instance members or non-static types â€” that requires Python type inference the codebase doesn't have, and would produce false positives on ordinary local variables.
...
**`max_tokens` cap + truncation detection:** `OpenAiLlmService` (base for OpenAI/DeepSeek/OpenRouter) sends `max_tokens: 8192` on every request (`OpenAiLlmService.MaxTokens`, `protected const`) â€” matching `ClaudeLlmService`'s existing cap. This does **not** affect generation quality below the cap (the model produces identical tokens either way); it only bounds worst-case response length, which matters because free-tier models in the OpenRouter fallback chain sometimes ignore the system prompt's "briefly explain, 2-3 sentences" rule and generate thousands of tokens of prose before the code. `OpenAiLlmService.WasTruncated` is set from the SSE stream's `finish_reason == "length"` and reset at the start of every `SendStreamingAsync` call. `CopilotPanelViewModel.RunStreamingAsync` checks it after streaming completes and appends a warning to `assistantVm.ValidationWarning` (the same banner used for enum/API validation issues) so a cut-off response is surfaced to the user instead of silently shipped as if it were complete. `OpenRouterLlmService.BuildRequestBody` overrides the base entirely (different `"models"` array shape) so it re-adds `max_tokens` itself — **do not** forget this if `BuildRequestBody` is overridden again elsewhere.

**Determinism (`temperature`) + served-model logging:** `OpenAiLlmService.BuildRequestBody` sends `temperature: 0.2` (`OpenAiLlmService.Temperature`, `protected const`) on every request — lower than each provider's own default (usually ~1.0) so code-gen output is more repeatable. `OpenRouterLlmService.BuildRequestBody` re-adds it for the same reason it re-adds `max_tokens` (its override replaces the whole dict). Separately, `OpenAiLlmService.LastServedModel` reads the `"model"` field off the first SSE chunk of each response and is reset at the start of every `SendStreamingAsync` call; `CopilotPanelViewModel.RunStreamingAsync` logs it via `CopilotLogger` after streaming completes. This matters specifically for OpenRouter: its fallback chain can silently serve a *different* model than the one requested if earlier entries in the chain are rate-limited or down, which looks like "the same prompt gave a wildly different answer" but is actually a different model answering — `LastServedModel` makes that visible in the log instead of it looking like unexplained inconsistency.

**429 rate-limit handling:** When OpenRouter's entire free-tier fallback chain is rate-limited (every model in the `models` array 429s), it returns HTTP 429 with a `previous_errors` array listing every model it tried. `OpenAiLlmService.SendStreamingAsync` detects `(int)response.StatusCode == 429` (checked as an int â€” `HttpStatusCode.TooManyRequests` is **not defined on net48**, same gotcha as the `StringSplitOptions` one below) and throws `LlmRateLimitException` (`DynamoCopilot.Core/Services/Providers/LlmRateLimitException.cs`) instead of the generic `HttpRequestException`. `BuildRateLimitException` parses `error.metadata.raw` + `error.metadata.retry_after_seconds` (OpenRouter's shape) or falls back to plain `error.message` (generic OpenAI-style shape). `CopilotPanelViewModel.RunStreamingAsync` catches `LlmRateLimitException` before the generic `catch (Exception ex)` and shows a clean "provider is rate-limited, try again in ~Ns" message instead of the raw provider JSON (which is unreadable â€” it includes the full `previous_errors` chain). **Do not** let this fall through to the generic catch â€” that's what dumps the raw JSON blob into the chat.
